...
models = [mode + '_ar', mode + '_armulti', mode + '_rf']
max_features = 52
...
if mode == 'forecasting':
    ...
    label = 'ARGO'
labels = [label, label + '-net LR', label + '-net RF']
...
if mode == 'forecasting':
    ...
    gs=GridSpec(3, 8, height_ratios=[1, 1, 1])
...
# Line plot
if mode == 'forecasting':
    line_plot_ax.set_title(str(th) + '-week Forecasts (Historical Epidemiological Data Only) for ILI in ' + loc, size='large')
else:
...
for i, model in enumerate(models):
    ...
    line_plot_ax.plot(runs[model]['dates'], runs[model]['yhats'], label=labels[i], color=colors[i], linewidth=3)
...
if mode == 'forecasting':
    lst = [(ar_heatmap_ax, None, models[0]), (armulti_heatmap_ax, None, models[1]), (rf_heatmap_ax, None, models[2])]
else:
...
for i, (ax, gt_ax, name) in enumerate(lst):
    ...
    if 'armulti' in name or 'rf' in name or 'lstm' in name:
        if 'armulti' in name:
        ...
        for feature, val in model['coefs'][index].items():
            if feature[:4] != 'term':
                ...
                print(feature)
                terms[gt_terms.index(feature.split('_')[-1])][list(df.columns).index(feature.split('_')[1])] = val
    else:
        title = ''
        xticklabels = [loc]
    ...
    ax.set_ylabel('Lag')
    if mode == 'nowcasting':
        gt_ax.set_title(title, size='large')
...
if mode == 'forecasting':
    letter_pos = [(0.01, 0.98), (0.81, 0.98), (0.01, 0.65), (0.01, 0.32)]
    plt.figtext(0.91, 0.98, 'Regression Coefficients in ' + labels[0], ha='center', va='center', size='large')
else:
