...
def lstm_with_trends(df, df_trends, th, n_test, long_test=False):
    ...
    def init_net(nodes):
        model = Sequential()
        model.add(Dropout(0.8, input_shape=(x_train.shape[1], x_train.shape[2])))
        model.add(GRU(best_nodes, input_shape=(x_train.shape[1], x_train.shape[2]), dropout=0.0000001))
